...
export default {
  ...
  watch: {
    ...
    "_selectedItem.roomId": {
      handler(val) {
        this.get({ url: "Rooms", id: this._selectedItem.roomId }).then(
          (data) => {
            this.selectedRoom = data;
            this.selectedRoom.roomPrices = this.selectedRoom.roomPrices.filter(
              (pr) =>
                this.getDate(this._selectedItem.dateOfEntry) >=
                  this.getDate(pr.seasonStartDate) &&
                this.getDate(this._selectedItem.exitDate) <=
                  this.getDate(pr.seasonEndDate)
            );
    ...
    "selectedRoom.seasonId": {
      handler(val) {
        console.log(val);
        this.seasonSelected = this.selectedRoom.roomPrices.find(
          (item) => item.lineSerial == val
        );
  ...
  methods: {
    ...
    async save() {
      if (!this._selectedItem.bookingTypeId) {
        this.doneAlert({
          text: this.$t("mustSelectBookingType"),
          type: "warning",
          timer: 10000,
        });
        return;
