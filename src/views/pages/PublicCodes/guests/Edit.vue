  ...
  <div class="main-wrapper">
    <div class="page-wrapper">
      <div class="content">
        <div class="card">
          <div class="card-body">
            <gform class="guest-data" @submit="save()">
              <!-- data tab -->
              <b-tabs content-class="pt-2" pills fill>
                <!-- for guest  -->
                <b-tab :title="$t('guestData')">
                  <b-row class="mt-1">
                    <!-- code  -->
                    <b-col v-if="_selectedItem.id > 0" md="3">
                      <gfield
                        label-text="code"
                        name="code"
                      ...
                      <b-form-group>
                        <div class="d-flex justify-content-between">
                          ...
                          <div v-if="_selectedItem.identityAttachmentName">
                            ...
                            >
                              <vue-feather type="download" size="12" />
                              {{ $t("download") }}
                            </b-button>
                            <b-button
                              v-b-tooltip.hover.top="
                                $t('removeIdentityAttachment')
                              "
                              variant="outline-danger"
                              style="font-size: 11px"
                              @click="deleteAttachment()"
                            >
                              <vue-feather type="trash-2" size="12" />
                              {{ $t("delete") }}
                            </b-button>
                          </div>
                        </div>
                        <div class="image-upload">
                          ...
                          <div class="image-uploads">
                            ...
                            <h4>
                              {{
                                _selectedItem.identityAttachmentName
                                  ? _selectedItem.identityAttachmentName
                                  : $t("dragAndDropFileToUpload")
                              }}
                            </h4>
                          </div>
                        </div>
                ...
                <b-tab :title="$t('companionsData')">
                  ...
                  <b-row class="mb-4 mt-2">
                    ...
                    <b-col md="3">
                      ...
                      <label
                        v-if="
                          _helper.companionValidator.showForName.required ||
                          _helper.companionValidator.showForName
                            .arabicOrEnglishOnly ||
                          _helper.companionValidator.showForName.unique
                        "
                    ...
                    <b-col md="3">
                      ...
                      <label
                        v-if="
                          _helper.companionValidator.showForRelativeRelation
                            .required ||
                          _helper.companionValidator.showForRelativeRelation
                            .arabicOrEnglishOnly
                        "
                    ...
                    <b-col md="3">
                      ...
                      <label
                        v-if="
                          _helper.companionValidator.showForIdentityContent
                            .required ||
                          _helper.companionValidator.showForIdentityContent
                            .numericOnly
                        "
                    ...
                    <b-col cols="12">
                      <b-button
                        ...
                        @click="reset()"
                      >
                        <vue-feather type="rotate-ccw" size="12" class="mx-1" />
                        {{ $t("reset") }}
                      </b-button>
                      ...
                      >
                        <vue-feather type="edit-3" size="12" class="mx-1" />
                        ...
                        <template #cell(actions)="{ item }">
                          <div class="text-nowrap">
                            ...
                            <b-button
                              ...
                              @click="edit(item)"
                            >
                              <vue-feather type="edit-3" size="12" />
                            </b-button>

                            <b-button
                              v-b-tooltip.hover.top="
                                $t('removeFromCompanionList')
                              "
                              variant="outline-danger"
                              size="sm"
...
<script>
// imported Files
import { genders, guestTypes, identityTypes } from "@/libs/acl/Lookups";
import { isStringOnly, isNumericOnly } from "@/mixin/publicMethods";
  ...
  computed: {
    ...
    getRulesForIdentityContent() {
      return {
        ...
        numeric:
          this._selectedItem.identityTypeId === "nationalIdentity" ||
    ...
    getValidatorMessageForComapnionName() {
      if (this._helper.companionValidator.showForName.required)
        return this.$t("required");
      else if (this._helper.companionValidator.showForName.arabicOrEnglishOnly)
        return this.$t("mustContainOnlyArabicOrEnglishCharcters");
      else if (this._helper.companionValidator.showForName.unique)
        return this.$t("companionNameAlreadyExists");
      else return "";
    },

    getValidatorMessageForComapnionRelationRelative() {
      if (this._helper.companionValidator.showForRelativeRelation.required)
        return this.$t("required");
      else if (
        this._helper.companionValidator.showForRelativeRelation
          .arabicOrEnglishOnly
    ...
    getValidatorMessageForComapnionIdentityContent() {
      if (this._helper.companionValidator.showForIdentityContent.required)
        return this.$t("required");
      else if (
        this._helper.companionValidator.showForIdentityContent.numericOnly
      )
        return this.$t("mustBeNumericOnly");
      else return "";
    },
  },
  ...
  methods: {
    handleFileInputChange(event) {
      if (event.target.files[0] && this.validateFile(event.target.files[0])) {
        ...
        this._selectedItem.identityAttachmentFileExtension = file.name
          .split(".")
          .pop()
          .toLowerCase();
        this.toBase64(file).then((fileBase) => {
          this._selectedItem.identityAttachmentBase64Content = fileBase
            .split(",")
            .pop();
        });
      } else {
    ...
    perpareForCreate() {
      if (!this._selectedItem.englishName)
        this._selectedItem.englishName = this._selectedItem.arabicName;
    },
    ...
    getSelectedItem() {
      this.get({ url: "Guests", id: this._id }).then((data) => {
        ...
        this._selectedItem.birthDateAd = new Date(
          this._selectedItem.birthDateAd
        );
        this._selectedItem.guestCompanions.forEach((element) => {
          ...
          this._companionsList.push({
            ...
            gender: genders.find((val) => val.id === element.genderId)
              .arabicName,
            identityType: !element.identityTypeId
              ? null
              : identityTypes.find((val) => val.id === element.identityTypeId)
                  .arabicName,
    ...
    getRulesForIdentityContentForCompanion() {
      return {
        isNumeric:
          this._companion.identityTypeId === "nationalIdentity" ||
    ...
    isCompanionUnique() {
      return !this._companionsList.find(
        (x) =>
          x.companionName === this._companion.companionName.trim() &&
          x.serialNo !== this._companion.serialNo
      );
    },
    ...
    isValidCompanion() {
      ...
      if (this._companion.identityTypeId && !this._companion.identityContent) {
        ...
        this._companion.identityTypeId &&
        this.getRulesForIdentityContentForCompanion().isNumeric &&
        !isNumericOnly(this._companion.identityContent)
      ) {
    ...
    addCompanionToList() {
      ...
      this._companionsList.push({
        ...
        gender: genders.find((val) => val.id === this._companion.genderId)
          .arabicName,
        identityType: !this._companion.identityTypeId
          ? null
          : identityTypes.find(
              (val) => val.id === this._companion.identityTypeId
            ).arabicName,
    ...
    preview(item) {
      this._companion = this._selectedItem.guestCompanions.find(
        (x) => x.companionName === item.companionName
      );
    ...
    removeFromList(item) {
      const elementIndex = this._selectedItem.guestCompanions.findIndex(
        (x) => x.companionName === item.companionName
      );
      if (elementIndex !== -1) {
    ...
    updateCompanion() {
      ...
      const elementIndex = this._selectedItem.guestCompanions.findIndex(
        (x) => x.companionName === this._companion.companionName
      );
      if (elementIndex !== -1) {
    ...
    downloadAttachment() {
      ...
      saveAs(
        `${this.baseUrl}${this._selectedItem.identityAttachmentUrl}`,
        `${this._selectedItem.identityAttachmentName}`
      );
    },
