<template>
    <div class="main-wrapper">
      <div class="page-wrapper">
        <div class="content">
          <div class="card">
            <div class="card-body">
              <gform class="accommodation-data">
                <b-tabs content-class="pt-2" pills fill>
                  <b-tab :title="$t('mainData')">
                    <b-row>
                      ...
                      <b-col md="3">
                        <gfield
                          label-text="pricePerNight"
                          name="pricePerNight"
                          v-model="_selectedItem.pricePerNight"
                          disabled
                        />
                      </b-col>
                      <b-col md="3">
                    ...
                    <b-row>
                      ...
                      <b-col md="3">
                        <gfield
                          ...
                          label="arabicName"
                          rules="required"
                          :options="roomPricesLookup"
                          name="season"
                          v-model="selectedRoom.seasonId"
                          field="select"
                          :dir="'rtl'"
                        />
                      </b-col>
                      <b-col md="3">
                        <gfield
                          label-text="seasonPrice"
                          id="seasonPrice"
                          label="arabicName"
                          rules="required"
                          :options="seasonPrices"
                          name="seasonPrice"
                          v-model="selectedRoom.seasonPrices"
                          field="select"
                          :dir="'rtl'"
                        />
                      </b-col>
  ...
  import { bookingType , seasonPrices } from "@/libs/acl/Lookups";
  ...
  export default {
    ...
    data: () => ({
      ...
      _selectedItem: {
        exitDate: new Date(new Date().getTime() + (24 * 60 * 60 * 1000)),
    ...
    watch: {
      ...
      "_selectedItem.roomId": {
        handler(val) {
          this.get({ url: "Rooms" , id:this. _selectedItem.roomId }).then((data) => {
            this.selectedRoom = data
            this.selectedRoom.roomPrices = this.selectedRoom.roomPrices.filter( (pr) => this.getDate(this._selectedItem.dateOfEntry)  >= this.getDate(pr.seasonStartDate) && this.getDate(this._selectedItem.exitDate) <= this.getDate(pr.seasonEndDate))
            this.selectedRoom.roomPrices.forEach(element => {
              this.roomPricesLookup.push({
                id: element.lineSerial,
                arabicName: element.seasonPriceArabicName
              })
            })
      ...
      "selectedRoom.seasonPrices": {
        handler(val) {
          console.log('seasonPrice', val);
          if (val = 'daily') {
            this._selectedItem.pricePerNight = this.seasonSelected.dailyPrice;
          }
        }
      },
      "selectedRoom.seasonId": {
        handler(val) {
          console.log(val);
          this.seasonSelected = this.selectedRoom.roomPrices.find((item) => item.lineSerial == val);
    ...
    mounted() {
      this._selectedItem.id = this.$route.params.id;
      if (this._selectedItem.id > 0) { 
             this.getSelected();
       }
    },
    methods: {
      priceSelected(val) {
        console.log('from watch thisFromPriceSelected', val);
      },
      calculatePrice() {
        console.log('ssss');
      },
      async save() {
        if (!this._selectedItem.bookingTypeId) {
          this.doneAlert({ text: this.$t('mustSelectBookingType'), type: 'warning', timer: 10000 });
        return;
        }
        if (!this._selectedItem.roomId) {
